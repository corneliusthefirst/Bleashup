...
export default class Message extends BeComponent {
  ...
  chooseComponent(data, index, sender) {
    const types = {
      ...
      [message_types.relation_message]: (
        <RelationMessage
  ...
  reaction(sender, element, index) {
    let color = !sender
      ? ColorList.transparentSentBox
      : ColorList.transparentReceivedBox;
    return (
      <View
        key={element.reaction + index}
        style={{ flexDirection: "column", marginLeft: 4 }}
      >
        <TouchableOpacity
          onPress={() =>
  ...
  render() {
    ...
    let transparentColor = this.state.sender
      ? ColorList.transparentReceivedBox
      : ColorList.transparentSentBox;
    let timeColor = transparentColor;
    let GeneralMessageBoxStyle = {
      maxWidth: GState.width * 0.78,
    ...
    return (
      ...
      >
        {this.props.message.type == message_types.date_separator ? (
          ...
          >
            <NewSeparator room={this.props.room}></NewSeparator>
          </View>
        ) : !this.state.loaded ? (
