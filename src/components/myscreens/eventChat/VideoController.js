...
export default class VideoController extends BeComponent {
  ...
  checkIFVideoExists(url) {
    return new Promise((resolve, reject) => {
      if (testForURL(url)) {
        Pickers.determineMediaDuration(url)
          .then((resp) => {
            resolve({
              status: Object.keys(resp).length ? true : false,
        ...
        new FileExachange().doFileExists(url).then((status) => {
          resolve({ status });
        });
      }
    });
   ...
   *  refreshes states when the called
   */
  assignState(mounted) {
  ...
  /**
   * determines the status of the player base of url provided to it.
   */
  initializePlayer() {
    this.checkIFVideoExists(this.props.source.uri).then((status) => {
      if (status.status) {
      ...
      <View style={styles.seekbar.container}>
        ...
        >
          <View
            style={[
              styles.seekbar.fill,
              {
                width: isNaN(this.state.seekerFillWidth)
                  ? "100%"
                  : this.state.seekerPosition,
                backgroundColor:
                  this.props.seekColor || ColorList.bodyBackground,
              },
            ]}
          />
        </View>
        <View
          style={[
            ...
            {
              left: isNaN(this.state.seekerPosition)
                ? "100%"
                : this.state.seekerPosition,
            },
          ]}
