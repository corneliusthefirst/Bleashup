...
export default class VideoMessage extends BePureComponent {
  ...
  componentDidMount() {
    let downloadState =
      (this.props.message.received / this.props.message.total) * 100;
    this.setStatePure({
      ...
      received: isNaN(parseInt(this.props.message.received))
        ? 0
        : parseInt(this.props.message.received),
      ...
      total: this.props.message.total
        ? parseInt(this.props.message.total).toFixed(2)
        : 0,
  ...
  onFail(received, total) {
    ...
    stores.Messages.addVideoSizeProperties(
      this.props.room,
  ...
  setAfterSuccess(res, cap, received) {
    stores.Messages.addVideoProperties(
      this.props.room,
      this.props.message.id,
      this.props.message.source,
      cap,
        ...
        >
          <View
            style={{
              ...
              minWidth: GState.messageMediaWidth,
            }}
          >
            <View>
              <CacheImages
              ...
              >
                {this.state.loaded ? (
                  <View style={{ marginTop: "-20%" }}>
                    <View style={{ ...buttoner, height: 25, width:null,minWidth:70 }}>
                      <Text style={{ color: ColorList.bodyBackground }}>
                        {this.toMB(this.state.total).toFixed(2)} {"Mb"}
                      </Text>
                    </View>
                  </View>
                ) : (
                  ...
                  >
                    <View style={{ height: 43 }}>
                      <AnimatedCircularProgress
                        ...
                        tintColor={
                          this.state.error ? "red" : ColorList.bodyBackground
                        }
                    ...
                    >
                      ...
                      >
                        {"("}
                        {this.toMB(this.state.received).toFixed(1)}
                        {"/"}
                        {this.toMB(this.state.total).toFixed(1)}
                        {")Mb"}
                      </Text>
                    </View>
                  </View>
