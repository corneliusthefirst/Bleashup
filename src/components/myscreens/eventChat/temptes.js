...
@observer class ChatRoom extends Component {
    ...
    showActions(message) {
        ActionSheet.show({ options: this.options, title: "Choose You Action", cancelButtonIndex: 4 }, (index) => {
            if (index === 0) {
                if (GState.connected) {
                    Requester.deleteMessage(message.id, this.props.activity_id, this.roomID)
                }
            } else if (index == 2) {
    ...
    render() {
        console.warn(stores.Messages.messages[this.roomID])
        headerStyles = {
    ...
    reactToMessage(messageID, reaction) {
        Requester.reactMessage(messageID, this.roomID, reaction, this.props.activity_id).then(() => {
    ...
    deleteMessage(messageID) {
        Requester.deleteMessage(messageID, this.props.activity_id, this.roomID).then(() => {
