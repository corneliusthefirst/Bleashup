...
export default class CreateEventView extends Component {
  ...
  creatEvent = ()=> {
    if (!this.state.currentEvent.about.title) {
      Toaster({ text: "The activity must have a name", duration: 4000 })
    } else {
      this.setState({
        creating: true
      })
      ...
      CreateRequest.createEvent(event).then((res) => {
        ...
        stores.Events.delete(request.Event().id).then(() => {
          firebase.database().ref(`rooms/${res.id}/${res.id}`).
          set({ name: 'General', members: res.participant }).then(() => {
            this.setState({
              currentEvent: request.Event(),
              title:"",
              photo:"",
    ...
    return (
      ...
      <View style={{flexDirection: "row",height: colorList.headerHeight,width: colorList.headerWidth,backgroundColor:colorList.headerBackground}}>
        <View style={{flex:1,backgroundColor:colorList.headerBackground,flexDirection: "row", alignItems: "flex-start",paddingLeft:"3%",...bleashupHeaderStyle
           ...
           <Text style={{color:colorList.headerText,fontSize:colorList.headerFontSize,fontWeight:colorList.headerFontweight ,                
