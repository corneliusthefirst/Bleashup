...
export default class CreateEventView extends Component {
  ...
  componentDidMount() {
    stores.Events.readFromStore().then(Events => {
      ...
      this.setState({
        participant: head(event.participant), currentEvent: event,
  ...
  TakePhotoFromLibrary = () => {
    return new Promise((resolve, reject) => {
      ...
      ImagePicker.openPicker({
        ...
        this.setState({ currentEvent: this.state.currentEvent });
        stores.Events.updateBackground(request.Event().id, res.path, false).then(() => { });
        resolve(res.path);
      });
    ...
    return (
      <View style={{ height: "100%", width: "100%" }}>
        <View style={{ flexDirection: "row", height: colorList.headerHeight, width: colorList.headerWidth, backgroundColor: colorList.headerBackground }}>
          <View style={{
            flex: 1, backgroundColor: colorList.headerBackground, flexDirection: "row", alignItems: "flex-start", paddingLeft: "3%", ...bleashupHeaderStyle
          }}>
            <View style={{ width: "10%", justifyContent: "center", height: "100%" }}>
              <MaterialIcons onPress={() => { this.props.navigation.navigate("Home") }}
                style={{ ...GState.defaultIconSize, color: colorList.headerIcon }} name={"arrow-back"} />
            </View>
            <View style={{ justifyContent: "center", height: "100%", width: "37%" }}>
              <Text style={{
                color: colorList.headerText, fontSize: colorList.headerFontSize, fontWeight: colorList.headerFontweight,
              }}>{Texts.new_activity}</Text>
            </View>
          </View>
        </View>
        <ScrollView style={{ height: "92%", flexDirection: "column" }}>
          ...
          <View style={{ height: colorList.containerHeight / 16, alignItems: "flex-end", flexDirection: "row", justifyContent: 'flex-end', alignItem: 'center', marginRight: "8%" }}>
            ...
            <EvilIcons name="camera" active={true} type="EvilIcons"
              style={{ color: colorList.bodyIcon, fontSize: 36 }} onPress={() => { this.TakePhotoFromCamera() }} />

            <MaterialIconCommunity name="download-outline" active={true} type="MaterialCommunityIcons"
              style={{ ...GState.defaultIconSize, color: colorList.bodyIcon }} onPress={() => { this.setState({ searchImageState: true }) }} />
          </View>
          <View style={{ height: 250, justifyContent: 'center' }}>
            <TouchableOpacity onPress={() => this.state.photo && testForURL(this.state.photo) ? this.setState({ enlargeImage: true }) : null} >
              {this.state.photo && testForURL(this.state.photo) ? <CacheImages thumbnails square source={{ uri: this.state.photo }}
                style={{
                  height: "90%", width: "81%", borderRadius: 5, alignSelf: "center",
                }} /> : <Image resizeMode={"cover"} source={this.state.photo ? { uri: this.state.photo } : this.state.DefaultPhoto}
                  style={{
            ...
            {this.state.photo ? <View style={{ position: 'absolute', alignSelf: 'flex-end', marginBottom: '60%', marginRight: '10%', }}>
              <TouchableOpacity onPress={() => {
                ...
                <EvilIcons name={'close'} type={'EvilIcons'} style={{ ...GState.defaultIconSize, color: 'red' }} />
              </TouchableOpacity>
            </View> : null}
