...
class SubPublicEvent extends Component {
    constructor(props) {
        super(props);
        this.state = {
            fetching: true,
    ...
    duration = 1
    openSwipeOut() {
        this.state.notPressing ? Vibration.vibrate(this.duration) : null
    ...
    render() {
        ...
        return (this.state.isMount ? <View style={{ width: "100%", }}>
                <Swipeout onOpen={() => this.openSwipeOut()}  onClose={() => this.onCloseSwipeout()} style={{ width:"100%",backgroundColor: this.props.Event.new ? "#cdfcfc" : null }} 
