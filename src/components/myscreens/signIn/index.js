...
export default class SignInView extends Component {
  ...
  componentWillMount() {
    firebase.auth().onAuthStateChanged(user => {
      console.warn("user from auth observe is: ", user)
      this.authUser = user
      if (user) {
  ...
  render() {
    return (
      <View style={{ height: "100%", width: "100%", alignItems: "center" }}>
        ...
        <ScrollView keyboardShouldPersistTaps={'hadled'} style={{ flex: 1, width: "100%" }} showsVerticalScrollIndicator={false}>
          <View style={{ ...rounder(200, ColorList.indicatorColor), marginLeft: "auto", marginRight: "auto", marginTop: "2%", }}>
            <Image resizeMode={"cover"} source={{ uri: this.loginStore.user.profile }}
              style={{ ...rounder(200), }}>
            </Image>
          </View>
          <View style={{
