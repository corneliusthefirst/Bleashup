...
export default class votes {
  ...
  fetchVotes(EventID) {
    return new Promise((resolve, reject) => {
      if (this.votes) {
        resolve(
          sortBy(filter(this.votes, { event_id: EventID })),
          "update_date"
        );
      } else {
        this.readFromStore().then(Votes => {
          resolve(sortBy(filter(Votes, { event_id: EventID })), "update_date");
        });
      }
    });
  }
  ...
  UpdateVoteDescription(NewVote, inform) {
    return new Promise((resolve, reject) => {
      this.readFromStore().then(Votes => {
        ...
        if (inform) Vote.description_updated = true;
        Vote.update_date = moment.format("YYYY-MM-DD HH:mm");
        Votes.splice(index, 1, Vote);
        this.saveKey.data = sortBy(Votes, "update_date");
        storage.save(this.saveKey).then(() => {
  ...
  UpdateVotePeriod(NewVote, inform) {
    return new Promise((resolve, reject) => {
      this.readFromStore().then(Votes => {
        ...
        if (inform) Vote.period_updated = true;
        Vote.update_date = moment.format("YYYY-MM-DD HH:mm");
        Votes.splice(index, 1, Vote);
        this.saveKey.data = sortBy(Votes, "update_date");
        storage.save(this.saveKey).then(() => {
  ...
  PublishVote(VoteID, inform) {
    return new Promise((resolve, reject) => {
      this.readFromStore().then(Votes => {
        ...
        let index = findIndex(Votes, { id: VoteID });
        Vote.published = true;
        Vote.update_date = moment.format("YYYY-MM-DD HH:mm");
        Votes.splice(index, 1, Vote);
        this.saveKey.data = sortBy(Votes, "update_date");
  ...
  removeVoteOption(VoteID, OptionName, inform) {
    return new Promise((resolve, reject) => {
      this.readFromStore().then(Votes => {
        ...
        if (inform) Vote.option_removed = true;
        Vote.update_date = moment.format("YYYY-MM-DD HH:mm");
        Votes.splice(index, 1, Vote);
        this.saveKey.data = sortBy(Votes, "update_date");
        storage.save(this.saveKey).then(() => {
