...
export default class StoriesStore {
  ...
  initializeGetStoriesListener() {
    ...
    emitter.on('give-story', (id, hid) => {
      console.warn(id, hid)
      if (this.curentTemporalStory.length <= 0 || this.curentTemporalStory[id].length <= 0) {
  ...
  removeStory(id) {
    return new Promise((resolve, rejectPromise) => {
      this.readFromStore().then(Stories => {
        ...
        this.saveKey.data =  id == "newStoryId" ?  [request.Story()].concat(Stories) : 
        Stories;
        storage.save(this.saveKey).then(() => {
          this.stories = this.saveKey.data;
  ...
  fetchStoriesFromRemote(statID) {
    return new Promise((resolve, reject) => {
      if (this.curentTemporalStory[statID] && this.curentTemporalStory[statID].length > 0) {
        resolve(this.curentTemporalStory[statID])
      } else {
        let statid = request.StatID()
        statid.stat_id = statID;
        tcpRequest.getStories(statid, statid.stat_id + "stories").then(JSONDATA => {
          serverEventListener.sendRequest(JSONDATA, statid.stat_id + "stories").then(Data => {
            if (Data.data === 'empty') {
              resolve([])
            } else {
              this.replaceStories(Array.isArray(Data.data) ?
                uniqBy(Data.data, 'id') : [Data.data], statID).then(() => {
